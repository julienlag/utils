...
#reading available file metadata from filename
my %fileMetadata;
open F, "$filesToIndex" or die "Could not open $filesToIndex. Exiting.\n";
while (<F>){
	chomp;
	my $fullPath=$_;
	my fileBasename=basename($fullpath);
	$fileBasename=
}
