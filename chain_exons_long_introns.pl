...
my %pslIndex=();
my $autoIncIndex=0;
my %queryseq_chr_qStart_qEnd_tStart_line=();
my $lastIndex=0;
my $counthits=0;
my $countperfecthits=0;
print STDERR "Scanning input PSL...\n";
while(<I>){
	$counthits++;
	chomp;
	my @line=split "\t";
	...
	if(exists $queryseq_chr_qStart_qEnd_tStart_line{$line[9]}{$line[13]}{$line[11]}{$line[12]}{$line[15]}){
		#die "$pslline\n";
		next;
	}
	
	$queryseq_chr_qStart_qEnd_tStart_line{$line[9]}{$line[13]}{$line[11]}{$line[12]}{$line[15]}=$autoIncIndex;
}
print STDERR "Scanning input PSL DONE\n";
...
my %currPslIndex=();
foreach my $queryseq (keys %queryseq_chr_qStart_qEnd_tStart_line){
	#print STDERR "\n$queryseq\t";
	foreach my $chr (keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}}){
		#print STDERR "$chr\t";
		#my @qStarts=();
		#my @qEnds=();
		#my $previous_qEnd=0;
		...
		my $chain_bool=0;
		my %tochain=();
		foreach my $qStart (sort {$a <=> $b} keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}} ){
			#$chain_bool=0;
			#print STDERR "qStart: $qStart\t";
			foreach my $qEnd (sort {$b <=> $a} keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}} ){
				#$previous_qEnd=$qEnd;
#				print STDERR "qEnd: $qEnd\t";
				
				foreach my $tStart (sort {$a <=> $b} keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}} ){
#					print STDERR "tStart: $tStart\t";
					foreach my $second_qStart(sort {$a <=> $b} keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}}){
#						print STDERR "second_qStart: $second_qStart\t";
						if($second_qStart>$qEnd-10 && $second_qStart<$qEnd+10){
						#if($second_qStart==$qEnd){

							foreach my $second_qEnd (sort {$b <=> $a} keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$second_qStart}} ){
								foreach my $second_tStart (sort {$a <=> $b}  keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$second_qStart}{$second_qEnd}} ){
									if((($second_tStart<$tStart && ${$pslIndex{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$second_qStart}{$second_qEnd}{$second_tStart}}}[8] eq '-') || ($second_tStart>$tStart && ${$pslIndex{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$second_qStart}{$second_qEnd}{$second_tStart}}}[8] eq '+')) && (${$pslIndex{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$second_qStart}{$second_qEnd}{$second_tStart}}}[8] eq ${$pslIndex{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}{$tStart}}}[8])){
										$second_index=$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$second_qStart}{$second_qEnd}{$second_tStart};
										#if(($previous_qEnd>0) && ($qStart>$previous_qEnd-10 && $qStart<$previous_qEnd+10)){
										$chain{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}{$tStart}}=$second_index;
										$tochain{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}{$tStart}}=1;
						...
						#else{
							#print STDOUT join("\t",@{$pslIndex{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}{$tStart}}})."\n";	
#							print STDERR "NO\t";
						#}
		...
		#else{
		foreach my $qStart (keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}} ){
			foreach my $qEnd (keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}} ){
				foreach my $tStart (keys %{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}} ){
					unless (exists ($tochain{$queryseq_chr_qStart_qEnd_tStart_line{$queryseq}{$chr}{$qStart}{$qEnd}{$tStart}})){
	...
	#delete $queryseq_chr_qStart_qEnd_tStart_line{$queryseq};
}
